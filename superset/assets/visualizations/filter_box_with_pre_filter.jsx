...
import './filter_box_with_pre_filter.css';
...
class FilterBoxWithPreFilterViz extends React.Component {
  constructor(props) {
    super(props);
    const sinceValue = moment(props.origSelectedValues[props.since]).format('YYYY-MM-DD');
    const untilValue = moment(props.origSelectedValues[props.until]).format('YYYY-MM-DD');
    let label = sinceValue + ' - ' + untilValue;
    if (sinceValue === untilValue) label = sinceValue;
    this.state = {
      selectedValues: props.origSelectedValues,
      hasChanged: false,
      startDate: moment(props.origSelectedValues[props.since]),
      endDate: moment(props.origSelectedValues[props.until]),
      dateLabel: label,
      ranges: {
        'Today': [moment(), moment()],
  ...
  dateFilterDateRangePickerApply(event, picker) {
    const sinceValue = moment(picker.startDate).format('YYYY-MM-DD');
    const untilValue = moment(picker.endDate).format('YYYY-MM-DD');
    let label = sinceValue + ' - ' + untilValue;
    if (sinceValue === untilValue) label = sinceValue;
    const selectedValues = Object.assign({}, this.state.selectedValues);
    selectedValues[this.props.since] = picker.startDate;
    selectedValues[this.props.until] = picker.startDate;
    this.setState({
    ...
    this.setState({ selectedValues, hasChanged: true });
    this.props.onChange(this.props.since, picker.startDate.format('YYYY-MM-DD'), false, this.props.instantFiltering);
    this.props.onChange(this.props.until, picker.endDate.format('YYYY-MM-DD'), false, this.props.instantFiltering);
  }
  render() {
    let dateFilterDateRangePicker;
    if (this.props.showDateFilter) {
      dateFilterDateRangePicker = (
        <div className="row space-1">
          <div className="col-lg-12 col-xs-12">
            <DateRangePicker
              ...
              onApply={this.dateFilterDateRangePickerApply.bind(this)}
            >
              <Button
              ...
              >
                ...
                <div className="pull-right">
                  <span className="pull-left">{this.state.dateLabel}</span>
                  <span className="caret pull-left" style={{ marginLeft: '10px' }} />
                </div>
              </Button>
            </DateRangePicker>
          </div>
...
FilterBoxWithPreFilterViz.propTypes = propTypes;
FilterBoxWithPreFilterViz.defaultProps = defaultProps;

function filterBoxWithPreFilterViz(slice, payload) {
...
module.exports = filterBoxWithPreFilterViz;
